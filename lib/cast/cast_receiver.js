 ...
 *   retrieve manifest URI from the provided content id.
  *  @param {function(Object):Object=} opt_customizedStatusCallback A
 * callback to modify the media information before to the connected senders.
...
shaka.cast.CastReceiver =
  ...
  /** @private {?function(string, Object):Promise} */
  this.opt_contentIdCallback_ = opt_contentIdCallback ||
                            /** @param {string} contentId
                                @param {Object} customData
                                * @return {Promise} */
                            function(contentId, customData) {
                              return Promise.resolve({});
                            };

  /** @private {?function(Object):Object} */
  this.opt_customizedStatusCallback_ = opt_customizedStatusCallback ||
 ...
 * @export
 */
shaka.cast.CastReceiver.prototype.getVideo = function() {
  return this.video_;
};
...
shaka.cast.CastReceiver.prototype.onShakaMessage_ = function(event) {
  ...
  shaka.log.debug('CastReceiver shaka: message', message);
  console.debug('CastReceiver shaka: message', message);
  switch (message['type']) {
    case 'init':
...
shaka.cast.CastReceiver.prototype.onGenericMessage_ = function(event) {
  ...
  console.debug('CastReceiver: message', message);
  ...
  switch (message['type']) {
    ...
    case 'LOAD':
      ...
      var customData = (message['customData']) ?
          message['customData'] :
          message['media'] ? message['media']['customData'] : undefined;
      this.opt_contentIdCallback_(contentId, customData).then(function(data) {
        console.debug('receivedData', data);
        var manifestUri = data['url'];
        var autoplay = message['autoplay'] || true;
        if (autoplay)
          this.video_.autoplay = true;
        if (data['drmKey'] !== undefined &&
            data['drmLicenseServer'] !== undefined) {
          var drmServer = {};
          drmServer[data['drmKey']] = data['drmLicenseServer'];
          var config = {
        ...
        if (manifestUri !== undefined) {
          console.debug('receivedManifest', manifestUri);
          this.player_.createMediaSource().then(function() {
...
shaka.cast.CastReceiver.prototype.sendMediaStatus_ =
  ...
  ret = this.opt_customizedStatusCallback_(ret);
